...
setTimeout(function () {
    if (!isSchool && gamePlays > 5 && Math.random() > 0.9) {
...
if (isSchool && game.spare == 2) {
    sessionExpiryTime = 100000; //100 seconds
    game.title += " [worthy]";
}

if (isSchool && game.spare == 3) {
    sessionExpiryTime = 100000; //100 seconds
    game.title += " [school test]";
}
...
if ((game.spare == 1 || game.spare == 4) && mustSetAnalytics) { //any 'extra' or 'test' games regardless of os
...
if (gamePlays > 2) {
    ...
    gtag('event', ("Loaded : " + game.title), {
        'event_category': 'GameA Play',
        'event_label': ('GameA : Loaded : ' + os)
    });

    setTimeout(function () {
        gtag('event', ("Session Expired : " + game.title), {
            'event_category': 'GameA Play',
            'event_label': ('GameA : Session Expired : ' + os)
        });
    }, sessionExpiryTime);

    setTimeout(function () {
        gtag('event', ("Session Expired 10 : " + game.title), {
            'event_category': 'GameA Play',
            'event_label': ('GameA : Session Expired 10 : ' + os)
        });
    }, 600000);
